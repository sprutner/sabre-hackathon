...
- **Sat (Phase 1 — transaction pipes): PARTIAL.**
  - **Pipe B (REST) LIVE:** session token works directly on `api.cert.platform.sabre.com` — no OAuth mint needed. `flightShop` SFO→SJD 2026-07-19: 50 offers in **2.2s**. Top-2 speakable verified (AM 699 via MEX, $280.08). `createBooking`/`getBooking`/`cancelBooking` implemented in `nightdesk/src/pipes/sabre-rest.ts` (classic ATPCO `flightDetails` path) — **booking NOT yet attempted, awaiting Seth's yes**.
  - **Pipe A (MCP connector) BLOCKED: Anthropic API key has zero credits** (400 "credit balance too low", both beta headers). Blocks pipe A comparison AND Phase 3 brain/model stopwatch. **Seth: add credits at console.anthropic.com → Plans & Billing.**
  - Architecture discovery: Sabre MCP workflow tools take only `conversationId` and return step-by-step instructions; actual calls go through `callSabreAPI` (13 whitelisted paths — full surface recorded in `sabre-tools-verbatim.json`). Spec tools mint a `conversationId` that must ride along on every `callSabreAPI` call. Direct REST bypasses all of this.
  - Q6 (sabre pipe) leaning REST: deterministic, 2.2s, no model in the loop for search. MCP comparison pending credits.
